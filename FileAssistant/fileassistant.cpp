...
namespace fileassistant {
    ...
    void FileAssistant::Initial()
    {
        ...
        connect(ui.btnSetting, &QPushButton::clicked, this, &FileAssistant::OnSetting);
	...
	void FileAssistant::OnSetting()
	{
        Settiongdlg dlg(this);
    ...
    void FileAssistant::EnableTableChecks(bool bEnable /*= true*/)
    {
        int nSize = m_upModel->rowCount();
        for (int i = 0; i < nSize; i++)
        {
            ...
            if (pItem)
            {
                pItem->setCheckable(bEnable);
            }
        }
    }

	void FileAssistant::UpdateSetting()
	{
		auto strPath1 = GetSettingValue(SETTING_SOURCE_PATH);
		if (!strPath1.isEmpty())
		{
			ui.editLocation1->setText(strPath1);
		}

		auto strPath2 = GetSettingValue(SETTING_TARGET_PATH);
		if (!strPath2.isEmpty())
		{
			ui.editLocation2->setText(strPath2);
		}

		auto font = GetSettingValue(SETTING_DEFAULT_FONT);
		this->setFont(font);
